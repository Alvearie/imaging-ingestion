...
public class QueryHelper {
    ...
    public class QueryBuilder {
        ...
        public QueryBuilder(Class<?> entity, String entityAlias, String... joins)  {
            queryString = String.format("SELECT DISTINCT %s FROM %s %s", entityAlias, entity.getSimpleName(), entityAlias);
        ...
        public void addParameter(String entityName, String parameterName, Object parameterValue) {
            queryString += String.format("%s %s = :%s", queryParameters.isEmpty() ? "" : " AND", entityName, parameterName);
        ...
        public void addListParameter(String entityName, String parameterName, List<Object> parameterValues) {
            queryString += String.format("%s %s IN (:%s)", queryParameters.isEmpty() ? "" : " AND", entityName, parameterName);
        ...
        public void addWildcardParameter(String entityName, String parameterName, Object parameterValue) {
            queryString += String.format("%s %s LIKE :%s", queryParameters.isEmpty() ? "" : " AND", entityName, parameterName);
            queryParameters.put(parameterName,  parameterValue);
        }
        
        public void addRangeParameter(String entityName, String parameterName, Object gtValue, Object ltValue) {
            queryString += String.format("%s %s BETWEEN :%s_GT AND :%s_LT", queryParameters.isEmpty() ? "" : " AND", entityName, parameterName, parameterName);
    ...
    public List<DicomStudyEntity> queryStudies(DicomQueryModel model) {
        ...
        PanacheQuery<DicomStudyEntity> query = queryBuilder.getParameters().isEmpty() ? DicomStudyEntity.findAll() : DicomStudyEntity.find(queryBuilder.getQueryString(), queryBuilder.getParameters());
    ...
    @SuppressWarnings("unchecked")
    public List<DicomSeriesEntity> querySeries(DicomQueryModel model) {
        QueryBuilder queryBuilder = new QueryBuilder(DicomSeriesEntity.class, "e", "attributes", "a", "instances", "i", "study.attributes", "sa");
        ...
        PanacheQuery<DicomSeriesEntity> query = queryBuilder.getParameters().isEmpty() ? DicomSeriesEntity.findAll() : DicomSeriesEntity.find(queryBuilder.getQueryString(), queryBuilder.getParameters());
        LOG.info("Query Result: " + query.count());  
    ...
    @SuppressWarnings("unchecked")
    public List<DicomInstanceEntity> queryInstances(DicomQueryModel model) {
        QueryBuilder queryBuilder = new QueryBuilder(DicomInstanceEntity.class, "e", "series.attributes", "a", "series.study.attributes", "sa");
        handlePatientIeAttributes(queryBuilder, "sa", model);
        if (model.getStudyUid() != null) {
        ...
        PanacheQuery<DicomInstanceEntity> query = queryBuilder.getParameters().isEmpty() ? DicomInstanceEntity.findAll() : DicomInstanceEntity.find(queryBuilder.getQueryString(), queryBuilder.getParameters());
        LOG.info("Query Result: " + query.count());
        query = (PanacheQuery<DicomInstanceEntity>) handlePagination(query, model);
        return query.list();
    }
    
    private void handlePatientIeAttributes(QueryBuilder queryBuilder, String studyAttributesAlias, DicomQueryModel model) {
        for (Map.Entry<Integer, String> attribute: model.getQueryAttributes().entrySet()) {
            ...
            case Tag.PatientName:
                ...
                if (model.isFuzzyMatching()) {
                    queryBuilder.addWildcardParameter(studyAttributesAlias + ".value", "value", attribute.getValue().replaceAll("\\*", "%"));
                } else {
    ...
    private void handleStudyIeAttributes(QueryBuilder queryBuilder, String studyAlias, String studyAttributesAlias, String seriesAlias, DicomQueryModel model) {
        for (Map.Entry<Integer, String> attribute: model.getQueryAttributes().entrySet()) {
            switch(attribute.getKey()) {
            case Tag.StudyInstanceUID:
                ...
                if (values.length > 1) {
                    ...
                    queryBuilder.addParameter(studyAlias + ".studyInstanceUID", "studyInstanceUID", attribute.getValue());
                }
                break;
            case Tag.StudyID:
    ...
    private void handleSeriesIeAttributes(QueryBuilder queryBuilder, String seriesAlias, String seriesAttributeAlias, DicomQueryModel model) {
        for (Map.Entry<Integer, String> attribute: model.getQueryAttributes().entrySet()) {
            switch(attribute.getKey()) {
            case Tag.Modality :
            ...
            case Tag.SeriesInstanceUID:
                ...
                if (values.length > 1) {
                    ...
                    queryBuilder.addParameter(seriesAlias + ".seriesInstanceUID", "seriesInstanceUID", attribute.getValue());
                }
                break;
        ...
        for (Map.Entry<Integer, String> attribute: model.getQueryAttributes().entrySet()) {
            ...
            case Tag.SOPInstanceUID:
                ...
                if (values.length > 1) {
                    ...
                    queryBuilder.addParameter(instanceAlias + ".sopInstanceUID", "sopInstanceUID", attribute.getValue());
                }
                break;
