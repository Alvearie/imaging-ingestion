...
var _ = BeforeSuite(func() {
	...
	testEnv = &envtest.Environment{
		CRDDirectoryPaths:     []string{filepath.Join("..", "config", "crd", "bases"), filepath.Join("..", "config", "test")},
	...
	err = imagingingestionv1alpha1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	err = kservingv1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	err = keventingv1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	err = ksourcesv1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
	...
	stateManager.SetState(common.KnativeServingKind, true)
	stateManager.SetState(common.KnativeEventingKind, true)
	...
	err = (&DicomEventBridgeReconciler{
		Client: k8sManager.GetClient(),
		Scheme: k8sManager.GetScheme(),
	}).SetupWithManager(k8sManager)
	Expect(err).ToNot(HaveOccurred())
